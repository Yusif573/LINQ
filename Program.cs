...
List<Debtor> debtors = new List<Debtor> {
            ...
            new Debtor("Adam D. Bartlett", DateTime.Parse("May 6, 1944"), "650-693-1758", "AdamDBartlett@rhyta.com", "2737 Rardin Drive San Jose, CA 95110", 5412),
            ...
            new Debtor("Martin S. Malinowski", DateTime.Parse("January 17, 1992"), "762-529-3523", "MartinSMalinowski@dayrep.com", "3749 Capitol Avenue New Castle, IN 47362", 1816),
...
//var result = debtors.Where(d => d.Email.Contains("rhyta.com") || d.Email.Contains("dayrep.com")).ToList();
//result.ForEach(d =>Console.WriteLine(d));

//	3) Yashi 26-dan 36-ya qeder olan borclulari cixartmag

//var result = debtors.Where(d => DateTime.Now.Year - d.BirthDay.Year > 26 && DateTime.Now.Year - d.BirthDay.Year < 36).ToList();
//result.ForEach(d => Console.WriteLine(d));

//	4) Borcu 5000-den cox olmayan borclularic cixartmag

//var result = debtors.Where(d => d.Debt < 5000).ToList();
//result.ForEach(d => Console.WriteLine(d));
...
//var result = debtors.Where(d => d.BirthDay.Month > 9 && d.BirthDay.Month < 13).ToList();
//result.ForEach(d => Console.WriteLine(d));
...
//double averageDebt = debtors.Sum(d => d.Debt) / debtors.Count();
////Console.WriteLine(averageDebt);
//var sorting = debtors.OrderBy(d => d.Debt).ToList();
//var result = sorting.Where(d => d.Debt > averageDebt).ToList();
//result.ForEach(d => Console.WriteLine(d));
...
//    Console.Write(" | ");
//    Console.Write(d.BirthDay);
//    Console.Write(" | ");
//    Console.Write(d.Debt);
//}
...
//var sorting = debtors.OrderBy(d => d.FullName.FirstOrDefault()).ToList();
//var hasThreeSameLetters = sorting.Where(d => d.FullName.GroupBy(c => c).Any(g => g.Count() >= 3)).ToList();
//hasThreeSameLetters.ForEach(d => Console.WriteLine(d));


//	13)borclulardan en coxu hansi ilde dogulubsa hemin ili cixartmaq

//var mostCommonYear = debtors.GroupBy(debtor => debtor.BirthDay.Year).OrderByDescending(group => group.Count()).First().Key;
//var result = debtors.Where(d => d.BirthDay.Year == mostCommonYear).ToList();
//result.ForEach(d => Console.WriteLine(d));

//	14)Borcu en boyuk olan 5 borclunun siyahisini cixartmaq

//var finding = debtors.OrderByDescending(d => d.Debt).Take(5).ToList();
//finding.ForEach(d => Console.WriteLine(d));

//	15)Butun borcu olanlarin borcunu cemleyib umumi borcu cixartmaq

//int result = debtors.Sum(d => d.Debt);
//Console.WriteLine(result);

//	18)Nomresinde tekrar reqemler olmayan borclularin ve onlarin borcunun meblegin cixartmaq

//var repeatingPhoneNumbers = debtors.GroupBy(debtor => debtor.Phone).Where(group => group.Count() > 2).Select(group => group.Key).ToList();

//var nonRepeatingPhoneNumbers = debtors.Select(debtor => debtor.Phone).Except(repeatingPhoneNumbers).ToList();
//nonRepeatingPhoneNumbers.ForEach(d => Console.WriteLine(d));
